...
const CityList = ({ data }) => {
  ...
  const handleLetterPress = (firstletter:string) => {
    ...
    const findindex = filteredCities.findIndex(city => 
  ...
  const sortCities = data.sort((a, b) => {
    return a.name.localeCompare(b.name);
  });
  const filteredCities = sortCities.filter((city) => {
  ...
  /* const renderSectionHeader = ({ section }) => {
    let firstletter = section.name as string;
    return (
      <Text style={{ fontSize: 20, fontWeight: "bold" }}>{firstletter}</Text>
    );
  };
